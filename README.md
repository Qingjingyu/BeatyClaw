...
BeatyClaw 现在以“数字员工”为主体。每个员工都会有独立实例目录：
...
- `runtime-instance.json`：产品层实例清单，说明这个员工的目录、引擎、容器名、状态和健康信息。
- `runtime-install.json`：AI 引擎安装清单，由具体 installer 写入，例如 HMS gateway 或 placeholder。
- `runtime-state.json`：运行状态，记录当前 status、health、port、pid、lastError。
- `logs/runtime.log`：该员工 runtime 进程的 stdout/stderr 日志。

接口返回的 employee 会带 `runtimeInstance`，用于前端和运维判断“这个员工背后对应哪套实例”。V1 是目录级真实实例，不代表已经为每个员工启动独立 Docker 容器；真实多容器部署属于后续 Runtime 生命周期阶段。

V2 已补强进程级生命周期：

- HMS / COCO / OpenClaw 的 process adapter 会为每个员工启动独立进程。
- 每个员工有自己的 pid、端口、runtime log。
...
tests/server/employees-service.test.ts                 员工实例闭环测试

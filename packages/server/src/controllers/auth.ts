...
import {
  buildYoyooClearCookie,
  buildYoyooSessionCookie,
  destroyYoyooSession,
  getYoyooSessionCookieName,
  getYoyooSpaceForUser,
  getYoyooUserBySession,
  loginYoyooUser,
} from '../services/yoyoo-auth'
...
export async function yoyooLogin(ctx: Context) {
  ...
  try {
    const loginResult = await loginYoyooUser(email, password)
    recordPasswordSuccess(ip)
    ctx.set('Set-Cookie', buildYoyooSessionCookie(loginResult.sessionToken))
    ctx.body = { user: loginResult.user }
  } catch {
...
export async function yoyooLogout(ctx: Context) {
  const sessionToken = ctx.cookies.get(getYoyooSessionCookieName())
  await destroyYoyooSession(sessionToken)
  ctx.set('Set-Cookie', buildYoyooClearCookie())
  ctx.body = { success: true }
}

export async function yoyooMe(ctx: Context) {
  const user = ctx.state?.yoyooUser || await getYoyooUserBySession(ctx.cookies.get(getYoyooSessionCookieName()))
  if (!user) {
    ctx.status = 401

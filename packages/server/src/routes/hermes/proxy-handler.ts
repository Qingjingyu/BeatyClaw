...
import { resolveYoyooGatewayForUser } from '../../services/yoyoo-gateway-manager'
...
function buildProxyHeaders(ctx: Context, upstream: string, apiKey?: string | null): Record<string, string> {
...
async function resolveProxyTarget(ctx: Context): Promise<{ profile: string; upstream: string; apiKey: string | null }> {
  const yoyooUser = ctx.state?.yoyooUser
  if (yoyooUser) {
    const endpoint = await resolveYoyooGatewayForUser(yoyooUser)
    return {
...
export async function proxy(ctx: Context) {
  ...
  try {
    const target = await resolveProxyTarget(ctx)
  ...
  const headers = buildProxyHeaders(ctx, upstream, apiKey)

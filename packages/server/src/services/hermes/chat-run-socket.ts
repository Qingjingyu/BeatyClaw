...
export class ChatRunSocket {
  ...
  private async handleRun(
    ...
    try {
      ...
      const requestBody = agenticRuntime
        ? buildAgenticChatCompletionBody(agenticRuntime, body, input)
        : body
      const requestPath = agenticRuntime ? '/v1/chat/completions' : '/v1/responses'
      ...
      let terminalOpenAIChunk: any | null = null
      for await (const frame of readSseFrames(res.body)) {
        let parsed: any
        if (agenticRuntime && frame.data.trim() === '[DONE]') {
          parsed = terminalOpenAIChunk || { id: responseId, choices: [{ finish_reason: 'stop' }] }
        } else {
          try {
        ...
        let upstreamEvent = parsed.type || frame.event || parsed.event
        if (agenticRuntime) {
          const mappedChunk = mapOpenAIChatCompletionChunk(parsed)
          if (!mappedChunk) continue
          if (mappedChunk.responseId || mappedChunk.model || mappedChunk.usage) {
            terminalOpenAIChunk = {
              id: mappedChunk.responseId || responseId,
              model: mappedChunk.model,
              usage: mappedChunk.usage,
              choices: [{ finish_reason: mappedChunk.finishReason || 'stop' }],
            }
          }
          if (mappedChunk.responseId) parsed.response = { id: mappedChunk.responseId, status: 'in_progress' }
          if (mappedChunk.delta) parsed.delta = mappedChunk.delta
          if (mappedChunk.model) parsed.model = mappedChunk.model
          if (mappedChunk.usage) parsed.usage = mappedChunk.usage
          if (mappedChunk.eventType === 'response.completed') {
            parsed.response = {
              id: mappedChunk.responseId || responseId,
              model: mappedChunk.model,
              usage: mappedChunk.usage,
          ...
          upstreamEvent = mappedChunk.eventType
        }

...
const { mockResolveYoyooGateway } = vi.hoisted(() => ({
  mockResolveYoyooGateway: vi.fn(),
}))
vi.mock('../../packages/server/src/services/yoyoo-gateway-manager', () => ({
  resolveYoyooGatewayForUser: mockResolveYoyooGateway,
}))
...
describe('Proxy Handler', () => {
  ...
  it('routes Yoyoo logged-in users through their own gateway', async () => {
    mockResolveYoyooGateway.mockResolvedValue({
      upstream: 'http://127.0.0.1:9650',
      apiKey: 'user-api-key',
      profile: 'yoyoo_abc123',
    })
    mockFetch.mockResolvedValue({
      status: 200,
      headers: new Headers({ 'content-type': 'application/json' }),
    ...
    expect(mockResolveYoyooGateway).toHaveBeenCalledWith(ctx.state.yoyooUser)
    expect(mockFetch.mock.calls[0][0]).toContain('http://127.0.0.1:9650/v1/runs')
    expect(mockFetch.mock.calls[0][1].headers.authorization).toBe('Bearer user-api-key')
  })
})

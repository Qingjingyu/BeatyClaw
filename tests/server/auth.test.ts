...
async function loadAuth(overrides: Partial<FsMocks> & { home?: string } = {}) {
  ...
  vi.doMock('../../packages/server/src/services/yoyoo-auth', () => ({
    getYoyooSessionCookieName: () => 'yoyoo_session',
    getYoyooUserBySession: vi.fn(async () => null),
  }))
...
describe('Auth Service', () => {
  ...
  describe('getToken', () => {
    ...
    it('returns null by default even when AUTH_TOKEN is set because legacy token auth is disabled', async () => {
    ...
    it('returns AUTH_TOKEN env var when legacy token auth is explicitly enabled', async () => {
      process.env.AGENTIC_ALLOW_TOKEN_AUTH = '1'
      process.env.AUTH_TOKEN = 'my-custom-token'
      const { getToken, mocks } = await loadAuth()
    ...
    it('reads token from file when legacy token auth is explicitly enabled', async () => {
      process.env.AGENTIC_ALLOW_TOKEN_AUTH = '1'
    ...
    it('generates and saves a token if legacy token auth is enabled and the token file is missing', async () => {
      process.env.AGENTIC_ALLOW_TOKEN_AUTH = '1'
